...
transactions_train['last_t_dat'] = transactions_train.groupby('user').t_dat.transform(max)
transactions_train['diff_t_dat'] = (transactions_train.last_t_dat - transactions_train.t_dat).dt.days
transactions_train = transactions_train.query("diff_t_dat <= 21").reset_index(drop=True)
...
OUTPUT_PATH_0 = './models/output/lightfm_index_only_fresh_0.csv'
...
OUTPUT_PATH_1 = './models/output/lightfm_index_only_fresh_1.csv'
pred['item'] = pred['item'].apply(lambda x: ' '.join(map(str, x)))
